...
WORDPRESS_DB_NAME=$(grep -Pom 1 "(?<=^WordpressDBName = ).*$" /root/NeXt-Server-Buster/wordpress_login_data.txt)
WordpressDBUser=$(grep -Pom 1 "(?<=^WordpressDBUser = ).*$" /root/NeXt-Server-Buster/wordpress_login_data.txt)
WordpressScriptPath=$(grep -Pom 1 "(?<=^WordpressScriptPath = ).*$" /root/NeXt-Server-Buster/wordpress_login_data.txt)
...
sed_replace_word "$WORDPRESS_PATH_NAME" "" "/root/NeXt-Server-Buster/configs/blocked_paths.conf"
