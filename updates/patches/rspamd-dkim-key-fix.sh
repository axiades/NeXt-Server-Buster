...
if grep --quiet "BEGIN PRIVATE KEY" /root/NeXt-Server-Buster/DKIM_KEY_ADD_TO_DNS.txt && \
   ...
   grep --quiet "BEGIN PRIVATE KEY" /var/lib/rspamd/dkim/${SELECTOR}.txt; then
    ...
    cp /root/NeXt-Server-Buster/DKIM_KEY_ADD_TO_DNS.txt "${BKUPP}/"
    chmod 600 -R "${BKUPP}"

    rspamadm dkim_keygen -b 2048 -s ${SELECTOR} -k /var/lib/rspamd/dkim/${SELECTOR}.key > /var/lib/rspamd/dkim/${SELECTOR}.txt
    chown -R _rspamd:_rspamd /var/lib/rspamd/dkim
    chmod 440 /var/lib/rspamd/dkim/*
    cp /var/lib/rspamd/dkim/${SELECTOR}.txt /root/NeXt-Server-Buster/DKIM_KEY_ADD_TO_DNS.txt
