...
cd /root/Perfectrootserver/sources/acme.sh/
...
echo "MAILSERVER_DB_PASS: $MAILSERVER_DB_PASS" >> /root/Perfectrootserver/login_information.txt
echo "#------------------------------------------------------------------------------#" >> /root/Perfectrootserver/login_information.txt
echo "" >> /root/Perfectrootserver/login_information.txt

sed_replace_word "placeholder" "${MAILSERVER_DB_PASS}" "/root/Perfectrootserver/configs/mailserver/database.sql"
mysql -u root -p${MYSQL_ROOT_PASS} mysql < /root/Perfectrootserver/configs/mailserver/database.sql

mysql -u root -p${MYSQL_ROOT_PASS} mysql < /root/Perfectrootserver/configs/mailserver/tlspolicies.sql
