...
cp /root/Perfectrootserver/configs/dovecot/dovecot.conf /etc/dovecot/dovecot.conf
sed_replace_word "domain.tld" "${MYDOMAIN}" "/etc/dovecot/dovecot.conf"

cp /root/Perfectrootserver/configs/dovecot/dovecot-sql.conf /etc/dovecot/dovecot-sql.conf
sed_replace_word "placeholder" "${MAILSERVER_DB_PASS}" "/etc/dovecot/dovecot-sql.conf"
chmod 440 /etc/dovecot/dovecot-sql.conf

cp /root/Perfectrootserver/configs/dovecot/sieve/* /var/vmail/sieve/global/
}
