...
mkdir -p /root/Perfectrootserver/sources/${FAIL2BAN_VERSION}/ >>"${main_log}" 2>>"${err_log}"
cd /root/Perfectrootserver/sources/${FAIL2BAN_VERSION}/ >>"${main_log}" 2>>"${err_log}"
...
cp /root/Perfectrootserver/configs/fail2ban/jail.local /etc/fail2ban/jail.local
...
rm -R /root/Perfectrootserver/sources/${FAIL2BAN_VERSION}
}
