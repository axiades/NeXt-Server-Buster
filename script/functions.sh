...
apt -y install $1 >>"${main_log}" 2>>"${err_log}" || error_exit "Failed to install $1 packages"
        ERROR=$?
        if [[ "$ERROR" != '0' ]]; then
...
show_login_information() {
  ...
  cat /root/Perfectrootserver/login_information.txt
}
...
continue_to_menu() {
  ...
  bash /root/Perfectrootserver/nxt.sh
}
